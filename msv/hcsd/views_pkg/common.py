...
_LOG_NOTE_PATTERNS = [
    ...
    (re.compile(r'^inspection_report:requirements_required$'), lambda m: 'نتيجة التفتيش: يتطلب اشتراطات إضافية.'),
    ...
    (re.compile(r'^Admin updated request data: (.+)$', re.DOTALL), lambda m: f'تم تحديث بيانات الطلب: {m.group(1)}'),
    (re.compile(r'^head_remarks:(.*)$', re.DOTALL), lambda m: (f'ملاحظات رئيس القسم: {m.group(1)}' if m.group(1) else 'تمت إضافة ملاحظات رئيس القسم.')),
